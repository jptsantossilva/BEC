...
- Telegram - Replaced the technical active-exchange prefix, such as `[exchange_id=2:kraken]`, with the readable exchange name, such as `Kraken`, while retaining the detailed identity in operational logs.
- Kraken Markets - Prevented legacy REP/REPV1 display aliases in Kraken's full public market catalog from blocking USDC market-phase scans; canonical market symbols now take precedence while both markets remain addressable by unambiguous identifiers.
...
- Database - Stopped normal application startup from repeating the exchange-aware backfill and table-rebuild migration, removing unnecessary SQLite write contention without changing the schema, timeout, or journal mode.
